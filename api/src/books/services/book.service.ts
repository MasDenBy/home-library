...
export class BookService {
  constructor(
    private dataStore: BookDataStore,
  ...
  public async search(dto: BookSearchDto): Promise<PageDto<BookDto>> {
    const searchResult = await this.dataStore.searchBooks(
      dto.pattern,
      dto.offset,
      dto.count,
    );
  ...
  public async getFile(id: number): Promise<[Stream, string]> {
    ...
    return [
      this.fs.readFileContent(book.file.path),
      this.fs.basenameExt(book.file.path),
    ];
  }
