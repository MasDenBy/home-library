...
describe('BookService', () => {
    ...
    beforeEach(() => {
        ...
        service = new BookService(instance(dataStoreMock), instance(fsMock), instance(openlibraryMock),
    ...
    test('search', async () => {
        ...
        when(dataStoreMock.searchBooks(dto.pattern, dto.offset, dto.count)).thenResolve([[book], 10]);

        // Act
        await service.search(dto);

        // Assert
        verify(dataStoreMock.searchBooks(dto.pattern, dto.offset, dto.count)).once();
    });
    ...
    describe('update', () => {
        ...
        test('should create metadata if it is null', async () => {
            ...
            // Assert
            verify(dataStoreMock.findByIdWithReferences(dto.id)).once();
            verify(dataStoreMock.update(objectContaining({metadata: {}} as Book))).once();
        });
    });
    ...
    describe('index', () => {
        ...
        describe('should', () => {
            ...
            test('create metadata if missing', () => {
                // Assert
                verify(dataStoreMock.update(objectContaining(<Book>{metadata: { isbn: isbn }}))).once();
            });
            ...
            test('update book with all details', () => {
                // Assert
                verify(dataStoreMock.update(objectContaining(
                    <Book>{
                        authors: authorName,
