...
export class OpenLibraryService {
    ...
    public async search(pattern: string): Promise<string> {
        const response = await axios.get<SearchResponseDto>(
            `${this.serviceUrl}/search.json?q=${OpenLibraryService.preparePattern(pattern)}`);
    ...
    public async findByIsbn(isbn: string): Promise<BookInfo> {
        const response = await axios.get(`${this.serviceUrl}/api/books?bibkeys=isbn:${isbn}&jscmd=details&format=json`);
